...
const CampaignApprovalPage = () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const mapped = apiPending.map(item => ({
          ...
          targetAmount: `Rp ${(item.target_amount || 0).toLocaleString('id-ID')}`,
  ...
  return (
    <div className="bg-[#F4F7F6] min-h-screen pb-20 pt-8">
                      ...
                      {selectedCampaign.rab.length > 0 ? selectedCampaign.rab.map((item, idx) => (
                        <div key={item.id || idx} className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between">
                          ...
                          <span className="font-bold text-slate-900 text-sm">
                            {typeof item.amount === 'number' ? `Rp ${item.amount.toLocaleString('id-ID')}` : item.amount}
                          </span>
                        </div>
                      )) : (
