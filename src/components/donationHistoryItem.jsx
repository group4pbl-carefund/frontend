...
const DonationHistoryItem = ({ donation, onDetailClick }) => {
  ...
  const categoryColor = isSuccess ? "text-emerald-600" : "text-amber-600";
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-grow">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <div className={`flex items-center gap-1 text-[10px] font-bold ${isSuccess ? 'text-emerald-500 bg-emerald-50' : 'text-amber-500 bg-amber-50'} px-2 py-0.5 rounded-full`}>
              {isSuccess ? <CheckCircle2 size={10} /> : <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>}
              <span>{isSuccess ? 'BERHASIL' : 'PENDING'}</span>
            </div>
          </div>
